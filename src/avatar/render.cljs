...
(def nose-gradient-scale-modifier
  "Global multiplier applied to all nose shade radial gradient scales."
  2.0)

(defn apply-nose-gradient-scale-modifier
  [gradient-transform]
  (if (and gradient-transform
           (not= nose-gradient-scale-modifier 1.0))
    (str/replace
     gradient-transform
     #"scale\(\s*([-0-9.]+)(?:\s+([-0-9.]+))?\s*\)"
     (fn [[_ sx sy]]
       (let [scaled-x (* (js/parseFloat sx) nose-gradient-scale-modifier)]
         (if sy
           (str "scale(" scaled-x " " (* (js/parseFloat sy) nose-gradient-scale-modifier) ")")
           (str "scale(" scaled-x ")")))))
    gradient-transform))
...
(defn make-nose
  [{:keys [path
           ...
           shade-path
           shade-node
           shade-gradient-transform
           preview-path
           preview-view-box
           preview-transform]}]
  (letfn [(skin-gradient-stops [skin]
            (:stops (get head-skin-gradients
                         (or skin :light-cream)
                         (get head-skin-gradients :light-cream))))
          (nose-shade-node [node skin]
            (if shade-gradient-transform
              (let [gradient-id (str "nose-skin-gradient-" (random-uuid))
                    [tag attrs] node]
                [:<>
                 [:defs
                 [:radialGradient
                   {:id gradient-id
                    ...
                    :r 1
                    :gradientUnits "userSpaceOnUse"
                    :gradientTransform (apply-nose-gradient-scale-modifier shade-gradient-transform)}
                   (for [{:keys [offset color]} (skin-gradient-stops skin)]
                     ^{:key (str offset "-" color)}
                     [:stop {:offset offset
                             :stop-color color}])]]
                 [tag (assoc (or attrs {}) :fill (str "url(#" gradient-id ")"))]])
              node))]
  (let [render-fn
        (fn nose-renderer [{:keys [mode skin fill]}]
          (if (= mode :preview)
            [:path {:d (or preview-path black-path path)
                    :fill (or fill "black")}]
            (asset-root
             (let [shade (or shade-node
                             (when shade-path [:path {:d shade-path}]))]
               (when shade
                 (nose-shade-node shade skin)))
...
(def nose-one
  (make-nose
   {:black-path "M33 51.5C33 45.2383 35.8834 40.412 38.3145 36.0361C40.8008 31.5607 43 27.3037 43 21.5C43 20.1193 44.1193 19 45.5 19C46.8807 19 48 20.1193 48 21.5C48 28.6961 45.1991 33.9394 42.6855 38.4639C40.1166 43.0879 38 46.7618 38 51.5C38 53.6684 38.6715 55.1553 39.5938 56.2422C40.5553 57.3754 41.9064 58.2055 43.4141 58.7979C44.9149 59.3875 46.4512 59.6943 47.6367 59.8496C48.2228 59.9264 48.7076 59.9639 49.0381 59.9824C49.192 59.9911 49.3467 60 49.501 60C50.8812 60.0005 52 61.1196 52 62.5C52 63.8807 50.8807 65 49.5 65C49.2529 65 49.0054 64.9884 48.7588 64.9746C48.3238 64.9502 47.7143 64.9017 46.9883 64.8066C45.5489 64.6182 43.5849 64.2374 41.5859 63.4521C39.5937 62.6695 37.4446 61.437 35.7812 59.4766C34.0786 57.4698 33 54.8316 33 51.5Z"
    ...
    :shade-gradient-transform "translate(50 49.5) rotate(-90) scale(19.5 12.408)"
...
(def nose-two
  (make-nose
   {:black-path "M22.9624 43.643C24.0886 42.6542 25.7528 42.8262 26.6801 44.027C33.7022 53.1196 41.7247 57.3663 49.6035 57.3663C57.5008 57.3663 65.8096 53.0989 73.377 43.9533C74.3422 42.7868 76.0122 42.6754 77.1061 43.7046C78.2 44.7339 78.3044 46.5147 77.3393 47.6812C69.0577 57.69 59.4042 63 49.6035 63C39.7842 63 30.3727 57.6692 22.6023 47.6075C21.6751 46.4066 21.8364 44.6319 22.9624 43.643Z"
    ...
    :shade-gradient-transform "translate(57.5 43) rotate(180) scale(26 14.0476)"
...
(def nose-four
  (make-nose
   {:black-path "M70.7213 43.2517C71.312 42.1 72.689 41.6643 73.7965 42.2785C74.9039 42.8929 75.3228 44.325 74.7322 45.4767C71.0251 52.7055 67.126 58.2829 63.0501 62.079C58.9599 65.8885 54.5793 68 50 68C45.4207 68 41.0401 65.8885 36.9499 62.079C32.874 58.2829 28.9749 52.7055 25.2678 45.4767C24.6772 44.325 25.0961 42.8929 26.2035 42.2785C27.311 41.6643 28.688 42.1 29.2787 43.2517C32.8441 50.2042 36.445 55.2631 39.9825 58.5577C43.5057 61.8389 46.8523 63.2728 50 63.2728C53.1477 63.2728 56.4943 61.8389 60.0175 58.5577C63.555 55.2631 67.1558 50.2042 70.7213 43.2517Z"
    ...
    :shade-gradient-transform "translate(52.5 44) rotate(90) scale(25 16.5803)"
...
(def nose-five
  (make-nose
   {:black-path "M40.1837 11.0012C41.6689 10.9579 42.9087 12.0982 42.9534 13.5483C43.6286 35.4758 42.133 45.304 35.3895 55.4382C33.3974 59.0052 34.1413 61.4961 37.6736 63.5889C43.0681 68.6563 54.0675 68.1285 60.1444 64.6582C62.6174 63.2459 64.7752 61.9266 65.27 60.7217C65.7529 59.5454 64.7857 57.0342 64.0324 54.8389C63.2677 52.6103 61.7752 49.9232 59.54 46.5923C58.7256 45.3786 59.0737 43.7505 60.3167 42.9554C61.5598 42.1604 63.2275 42.4991 64.0419 43.7127C66.3518 47.155 68.1483 50.2931 69.1356 53.1703C70.1342 56.0805 71.3822 59.9567 70.2649 62.6776C69.1592 65.3699 65.9004 67.4586 62.8615 69.1941C53.0938 74.7721 43.243 73.122 33.9421 67.3746C28.8844 64.2492 28.1491 59.8486 30.0425 54.4767L30.0572 54.2991C30.1074 53.8883 30.2568 53.494 30.4934 53.1488C36.6138 44.225 38.2576 35.8773 37.5748 13.7063C37.5301 12.256 38.6982 11.0448 40.1837 11.0012Z"
    ...
    :shade-gradient-transform "translate(53 43) rotate(88.3634) scale(35.0143 11.0319)"
...
(def nose-seven
  (make-nose
   {:black-path "M68.5509 33.3518C69.3493 31.9796 70.9964 31.5873 72.2297 32.4753C77.2256 36.0739 80.4251 39.7009 82.0991 43.3741C83.8233 47.1577 83.837 50.7985 82.786 54.0139C81.7716 57.1174 79.8265 59.6569 77.856 61.5931C75.8683 63.5462 73.7007 65.0365 72.0001 66.0088C69.9539 67.1785 66.5067 67.876 62.8353 68.3167C59.0192 68.7747 54.5042 69 50.0125 69C45.5203 69 40.9951 68.7745 37.1585 68.3167C33.4582 67.8751 29.9965 67.1784 27.9209 66.0238C24.4466 64.091 19.1666 59.6846 17.2337 53.5664C16.2233 50.368 16.1512 46.747 17.6858 43.0122C19.1875 39.3572 22.126 35.8375 26.7175 32.4857C27.946 31.5891 29.5945 31.9699 30.4005 33.3367C31.2064 34.7035 30.864 36.5377 29.6356 37.4345C25.5531 40.4147 23.4725 43.1686 22.5254 45.4739C21.6111 47.699 21.6597 49.7247 22.2531 51.6031C23.5281 55.6389 27.3901 59.109 30.3007 60.7282C31.4178 61.3496 33.9848 61.985 37.7259 62.4314C41.3309 62.8615 45.6609 63.08 50.0125 63.08C54.3641 63.08 58.6825 62.8613 62.2647 62.4314C65.9913 61.9841 68.508 61.3493 69.5683 60.7432C70.9741 59.9395 72.7511 58.7119 74.3341 57.1566C75.9343 55.5843 77.1868 53.8248 77.7864 51.9905C78.3494 50.268 78.3934 48.3315 77.3541 46.0508C76.2643 43.6596 73.9052 40.7342 69.3386 37.4449C68.1054 36.5565 67.7527 34.724 68.5509 33.3518Z"
    :shade-node [:ellipse {:cx 50 :cy 48.5 :rx 32 :ry 18.5}]
    :shade-gradient-transform "translate(62.5 49) rotate(90) scale(16.5 36.1229)"
...
(def nose-eight
  (make-nose
   {:black-path "M50.7829 32.875C61.572 33.0134 72.0612 39.6848 75.8923 52.2725C76.2941 53.5933 75.549 54.9896 74.2282 55.3916C72.9074 55.7934 71.5111 55.0483 71.1091 53.7275C67.94 43.3155 59.4292 37.9867 50.7185 37.875C42.0193 37.7636 32.9704 42.8676 28.8415 53.8779C28.3567 55.1707 26.9155 55.8256 25.6227 55.3408C24.33 54.856 23.6751 53.4148 24.1599 52.1221C29.031 39.1326 39.9822 32.7365 50.7829 32.875Z"
    ...
    :shade-gradient-transform "translate(57 46.25) rotate(90) scale(9 30)"
...
(def nose-nine
  (make-nose
   {:black-path "M59.25 13.1353C60.7615 12.6745 62.3927 13.4288 62.8933 14.8204C67.2723 26.9993 70.4854 36.7368 70.9434 45.4333C71.4123 54.3391 68.9997 62.087 62.4856 70.3127C62.1106 70.7863 61.5762 71.1324 60.9652 71.2968C57.0255 72.357 53.7442 72.9805 50.371 72.9996C46.984 73.0186 43.6863 72.4278 39.6946 71.2833C39.1289 71.1211 38.6308 70.8027 38.2688 70.3708C30.514 61.1165 28.2842 52.806 29.1893 43.8332C30.0629 35.1736 33.8979 25.8584 37.8104 14.8308C38.3049 13.4371 39.9334 12.6769 41.447 13.1322C42.9603 13.5876 43.7861 15.086 43.2917 16.4796C39.2219 27.9503 35.7266 36.4349 34.9307 44.3237C34.1874 51.6924 35.8353 58.5041 42.2803 66.4541C45.5079 67.3302 47.9591 67.7036 50.3361 67.6902C52.7101 67.6768 55.1602 67.2772 58.3986 66.4468C63.7289 59.4511 65.5729 53.0761 65.184 45.6905C64.7704 37.8372 61.8406 28.7844 57.4199 16.4899C56.9195 15.0982 57.7386 13.5962 59.25 13.1353Z"
    ...
    :shade-gradient-transform "translate(57.5 50.5) rotate(-90) scale(55.5 31)"
...
(def nose-ten
  (make-nose
